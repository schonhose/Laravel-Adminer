...
adm_cookie($B,$Y,$ve=2592000){global$ba;return
...
adm_redirect($xe,$Le=null){if($Le!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($xe!==null?$xe:$_SERVER["REQUEST_URI"]))][]=$Le;}if($xe!==null){if($xe=="")$xe=".";header("Location: $xe");exit;}}function
query_adm_redirect($F,$xe,$Le,$Bg=true,$Cc=true,$Nc=false,$ei=""){global$g,$n,$b;if($Cc){$Dh=microtime(true);$Nc=!$g->query($F);$ei=format_time($Dh);}$zh="";if($F)$zh=$b->messageQuery($F,$ei,$Nc);if($Nc){$n=error().$zh.script("messagesPrint();");return
false;}if($Bg)adm_redirect($xe,$Le.$zh);return
true;}function
...
false;}return
true;}function
queries_adm_redirect($xe,$Le,$Bg){list($vg,$ei)=queries(null);return
query_adm_redirect($vg,$xe,$Le,$Bg,false,!$Bg,$ei);}function
...
limit($F,$Z,$z,$C=0,$L=" "){return" $F$Z".($z!==null?$L."LIMIT $z".($C?" OFFSET $C":""):"");}function
limit1($Q,$F,$Z,$L="\n"){return(preg_match('~^INTO~',$F)?limit($F,$Z,1,0,$L):" $F".(is_adm_view(table_status1($Q))?$Z:" WHERE ctid = (SELECT ctid FROM ".table($Q).$Z.$L."LIMIT 1)"));}function
...
false;}}foreach($aj
as$Q){$B=($Xh==DB?table("copy_$Q"):idf_escape($Xh).".".table($Q));$Zi=adm_view($Q);if(($_POST["overwrite"]&&!queries("DROP VIEW IF EXISTS $B"))||!queries("CREATE VIEW $B AS $Zi[select]"))return
false;}return
true;}function
trigger($B){if($B=="")return
...
selectLinks($Oh,$N=""){global$x,$m;echo'<p class="links">';$we=array("select"=>'Select data');if(support("table")||support("indexes"))$we["table"]='Show structure';if(support("table")){if(is_adm_view($Oh))$we["view"]='Alter view';else$we["create"]='Alter table';}if($N!==null)$we["edit"]='New item';$B=$Oh["Name"];foreach($we
...
adm_cookie("adminer_settings",http_build_query($ph+adminer_settings()));}function
...
drop_create($fc,$i,$gc,$bi,$ic,$xe,$Oe,$Me,$Ne,$mf,$Ze){if($_POST["drop"])query_adm_redirect($fc,$xe,$Oe);elseif($mf=="")query_adm_redirect($i,$xe,$Ne);elseif($mf!=$Ze){$Gb=queries($i);queries_adm_redirect($xe,$Me,$Gb&&queries($fc));if($Gb)queries($gc);}else
queries_adm_redirect($xe,$Me,queries($bi)&&queries($ic)&&queries($fc)&&queries($i));}function
...
TmpFile;ob_start(array($li,'write'),1e5);}$b->dumpTable($B,($Q?$_POST["table_style"]:""),(is_adm_view($R)?2:0));if(is_adm_view($R))$aj[]=$B;elseif($Mb){$p=fields($B);$b->dumpData($B,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($B));}if($Zd&&$_POST["triggers"]&&$Q&&($_i=trigger_sql($B)))echo"\nDELIMITER ;;\n$_i\nDELIMITER ;\n";if($Ic=="tar"){ob_end_flush();tar_file((DB!=""?"":"$l/")."$B.csv",$li);}elseif($Zd)echo"\n";}}foreach($aj
...
as$B=>$o){if(!isset($o["privileges"][$Ji?"update":"insert"])||$b->fieldName($o)==""||$o["generated"])unset($p[$B]);}if($_POST&&!$n&&!isset($_GET["select"])){$xe=$_POST["referer"];if($_POST["insert"])$xe=($Ji?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$xe))$xe=ME."select=".urlencode($a);$w=indexes($a);$Ei=unique_array($_GET["where"],$w);$xg="\nWHERE $Z";if(isset($_POST["delete"]))queries_adm_redirect($xe,'Item has been deleted.',$m->delete($a,$xg,!$Ei));else{$N=array();foreach($p
as$B=>$o){$X=process_input($o);if($X!==false&&$X!==null)$N[idf_escape($B)]=$X;}if($Ji){if(!$N)adm_redirect($xe);queries_adm_redirect($xe,'Item has been updated.',$m->update($a,$N,$xg,!$Ei));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$G=$m->insert($a,$N);$oe=($G?last_id():0);queries_adm_redirect($xe,sprintf('Item%s has been inserted.',($oe?" $oe":"")),$G);}}}$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($p
...
';}elseif(isset($_GET["database"])){$I=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$B=trim($I["name"]);if($_POST["drop"]){$_GET["db"]="";queries_adm_redirect(remove_from_uri("db|database"),'Database has been dropped.',drop_databases(array(DB)));}elseif(DB!==$B){if(DB!=""){$_GET["db"]=$B;queries_adm_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($B),'Database has been renamed.',rename_database($B,$I["collation"]));}else{$k=explode("\n",str_replace("\r","",$B));$Jh=true;$ne="";foreach($k
as$l){if(count($k)==1||$l!=""){if(!create_database($l,$I["collation"]))$Jh=false;$ne=$l;}}restart_session();set_session("dbs",null);queries_adm_redirect(ME."db=".urlencode($ne),'Database has been created.',$Jh);}}else{if(!$I["collation"])adm_redirect(substr(ME,0,-1));query_adm_redirect("ALTER DATABASE ".idf_escape($B).(preg_match('~^[a-z0-9_]+$~i',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Database has been altered.');}}page_header(DB!=""?'Alter database':'Create database',$n,array(),h(DB));$pb=collations();$B=DB;if($_POST)$B=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$pb);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$ld){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ld,$A)&&$A[1]){$B=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["scheme"])){$I=$_POST;if($_POST&&!$n){$_=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_adm_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$_,'Schema has been dropped.');else{$B=trim($I["name"]);$_.=urlencode($B);if($_GET["ns"]=="")query_adm_redirect("CREATE SCHEMA ".idf_escape($B),$_,'Schema has been created.');elseif($_GET["ns"]!=$B)query_adm_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($B),$_,'Schema has been altered.');else
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$B=$_GET["name"];$I=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$Le=($_POST["drop"]?'Foreign key has been dropped.':($B!=""?'Foreign key has been altered.':'Foreign key has been created.'));$xe=ME."table=".urlencode($a);if(!$_POST["drop"]){$I["source"]=array_filter($I["source"],'strlen');ksort($I["source"]);$Xh=array();foreach($I["source"]as$y=>$X)$Xh[$y]=$I["target"][$y];$I["target"]=$Xh;}if($x=="sqlite")queries_adm_redirect($xe,$Le,recreate_table($a,$a,array(),array(),array(" $B"=>($_POST["drop"]?"":" ".format_foreign_key($I)))));else{$c="ALTER TABLE ".table($a);$fc="\nDROP ".($x=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($B);if($_POST["drop"])query_adm_redirect($c.$fc,$xe,$Le);else{query_adm_redirect($c.($B!=""?"$fc,":"")."\nADD".format_foreign_key($I),$xe,$Le);$n='Source and target columns must have the same data type, there must be an index on the target columns and referenced data must exist.'."<br>$n";}}}page_header('Foreign key',$n,array("table"=>$a),h($a));if($_POST){ksort($I["source"]);if($_POST["add"])$I["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$I["target"]=array();}elseif($B!=""){$ed=foreign_keys($a);$I=$ed[$B];$I["source"][]="";}else{$I["table"]=$a;$I["source"]=array("");}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$I=$_POST;if($_POST&&!$n){$_=substr(ME,0,-1);$B=trim($I["name"]);if($_POST["drop"])query_adm_redirect("DROP SEQUENCE ".idf_escape($fa),$_,'Sequence has been dropped.');elseif($fa=="")query_adm_redirect("CREATE SEQUENCE ".idf_escape($B),$_,'Sequence has been created.');elseif($fa!=$B)query_adm_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($B),$_,'Sequence has been altered.');else
...
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$I=$_POST;if($_POST&&!$n){$_=substr(ME,0,-1);if($_POST["drop"])query_adm_redirect("DROP TYPE ".idf_escape($ga),$_,'Type has been dropped.');else
query_adm_redirect("CREATE TYPE ".idf_escape(trim($I["name"]))." $I[as]",$_,'Type has been created.');}page_header($ga!=""?'Alter type'.": ".h($ga):'Create type',$n);if(!$I)$I["as"]="AS ";echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$B=$_GET["name"];$zi=trigger_options();$I=(array)trigger($B)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$zi["Timing"])&&in_array($_POST["Event"],$zi["Event"])&&in_array($_POST["Type"],$zi["Type"])){$pf=" ON ".table($a);$fc="DROP TRIGGER ".idf_escape($B).($x=="pgsql"?$pf:"");$xe=ME."table=".urlencode($a);if($_POST["drop"])query_adm_redirect($fc,$xe,'Trigger has been dropped.');else{if($B!="")queries($fc);queries_adm_redirect($xe,($B!=""?'Trigger has been altered.':'Trigger has been created.'),queries(create_trigger($pf,$_POST)));if($B!="")queries(create_trigger($pf,$I+array("Type"=>reset($zi["Type"]))));}}$I=$_POST;}page_header(($B!=""?'Alter trigger'.": ".h($B):'Create trigger'),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$je=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$je++;}queries_adm_redirect(ME."processlist=",lang(array('%d process has been killed.','%d processes have been killed.'),$je),$je||!$_POST["kill"]);}page_header('Process list',$n);echo'
<form action="" method="post">
<div class="scrollable">
